...
export default function AuthPage({ mode }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-fundsphere-gray px-4 py-8">
      <section className="grid w-full max-w-5xl overflow-hidden rounded-lg bg-white shadow-soft md:grid-cols-[0.9fr_1.1fr]">
        <div className="bg-fundsphere-blue p-8 text-white">
          <Logo />
        ...
        <div className="p-6 md:p-8">
          <div className="mb-6 flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-50 text-fundsphere-blue">
            ...
            <div>
              ...
              <p className="text-slate-600">{isRegister ? 'Start your digital financial locker.' : 'Welcome back to FundSphere.'}</p>
            </div>
          </div>
          {error && <div className="mb-4 rounded-lg border border-red-200 bg-red-50 p-4 font-semibold text-fundsphere-red">{error}</div>}
          <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
            ...
            {isRegister ? (
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <button type="button" className="font-semibold text-fundsphere-blue">Forgot Password</button>
              </div>
            )}
          ...
          <p className="mt-6 text-center text-base text-slate-600">
            {isRegister ? 'Already have an account?' : 'New to FundSphere?'}{' '}
            <Link className="font-bold text-fundsphere-blue" to={isRegister ? '/login' : '/register'}>{isRegister ? 'Login' : 'Register'}</Link>
          </p>
        </div>
      </section>
    </main>
